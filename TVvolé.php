...
if(isset($_GET['nums'])){
	$getnums=$_GET['nums'];
	$req=$pdo->prepare("SELECT * FROM tv WHERE num_s= ?");
	$req->execute(array($getnums));
	$info=$req->fetch();
 if($info['num_s']==$_SESSION['num_s']){
  $ID=$info['id'];
...
<body>
  ...
  <section class="banner">
    <header>
      <a href="detection-TV.php?num=<?=$number?>" class="re" ><i id ="icon"class="fas fa-arrow-alt-circle-left"></i> Retour</a>
   
    </header>  
     <h2>attention le TV qui vous interesse a était déja volé  <script src="https://cdn.lordicon.com/lusqsztk.js"></script>
<lord-icon
...
<div class="container">
  ...
  <img  src="TV-déclarés/<?php echo $info['photo'];?>" height="280px" width="330px">
    <ul>
        <li> <p>type du TV: </p><?php echo $info['typeTV'];?> </li>
        <li> <p>Taille en pouce :</p> <?php echo $info['taille'];?></li>
