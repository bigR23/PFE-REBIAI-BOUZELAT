...
if(isset($_GET['nums'])){
	$getnums=$_GET['nums'];
	$req=$pdo->prepare("SELECT * FROM Laptop WHERE num_s= ?");
	$req->execute(array($getnums));
	$info=$req->fetch();
 if(isset($_SESSION['num_s'])AND $info['num_s']==$_SESSION['num_s']){
  $ID=$info['id'];
...
<body>
  ...
  <section class="banner">
    <header>
      <a href="detection-laptop.php?num=<?=$number?>" class="re" ><i id ="icon"class="fas fa-arrow-alt-circle-left"></i> Retour</a>
   
    </header>  
     <h2>attention le Laptop qui vous interesse a était déja volé  <script src="https://cdn.lordicon.com/lusqsztk.js"></script>
<lord-icon
...
<div class="container">
  ...
  <img  src="laptops-déclarés/<?php echo $info['photo'];?>" height="280px" width="330px">
    <ul>
        <li> <p>nom: </p><?php echo $info['nom_appareil'];?> </li>
        <li> <p>id de modèle:</p> <?php echo $info['id_modele'];?></li>
   ...
   <div class="alerte"id="alerte"><p> Bonjour Monsieur/Madame<br> Vous avez détécter un objet volé donc s'il vous plait n'hésitez pas a informer son propriétaire<p>
      <p><i class="fa-solid fa-phone" id="icon"></i>&emsp;<?php echo $num;?></p> </div>
