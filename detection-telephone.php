...
$reqtel=$pdo->prepare("SELECT *FROM telephone WHERE emei= ? AND confirmation ='OK'");
$reqtel->execute(array($rech));
$telephoneexist= $reqtel->rowCount();

if($telephoneexist==1){

$telinfo=$reqtel->fetch();

$_SESSION['emei']=$telinfo['emei'];

$_SESSION['marque']=$telinfo['marque'];

$_SESSION['nom_appareil']=$telinfo['nom_appareil'];

$_SESSION['num_s']=$telinfo['num_s'];

$_SESSION['couleur']=$telinfo['couleur'];

$_SESSION['photo']=$telinfo['photo'];
...
<link rel="stylesheet" href="css/all.css"/>
		...
		align="center">
			<br />
			<p style="font-size:1.3em;color:#fff ;margin: 20px 0px;text-shadow:0 0 4px #000000;"> Votre téléphone a été retrouvé par un utilisateur de notre application,
...
mail($mail, "Votre téléphone a été retrouvé !", $message, $header);


header("Location:téléphone-volé.php?num=$number&emei=".$_SESSION['emei']);

}else{
  header("Location:objet-n'est-pas-déclaré.php?num=$number&objet=le téléphone");
}
}
else {
...
<head>
  ...
  <title>Détection du téléphone</title>
  <link rel="stylesheet" href="css/all.css"/>
...
?> </header>
  <h2>Détection du téléphone par Imei  <script src="https://cdn.lordicon.com/lusqsztk.js"></script>
    <lord-icon
