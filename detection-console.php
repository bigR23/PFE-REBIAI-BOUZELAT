...
$reqapp=$pdo->prepare("SELECT *FROM console WHERE num_s= ? AND confirmation='OK'");
...
<link rel="stylesheet" href="css/all.css"/>
		...
		align="center">
			<br />
			<p style="font-size:1.3em;color:#fff ;margin: 20px 0px;text-shadow:0 0 4px #000000;"> Votre console a été retrouvée par un utilisateur de notre application,
...
mail($mail, "Votre console a été retrouvée !", $message, $header);

header("Location:console-volé.php?num=$number&nums=".$_SESSION['num_s']);

}else{
  header("Location:objet-n'est-pas-déclaré.php?num=$number&objet=la console");
}
}
else {
...
<head>
  ...
  <title>Détection de console</title>
  <link rel="stylesheet" href="css/all.css"/>
...
?> </header>
  <h2>Détection de console par numéro de série<script src="https://cdn.lordicon.com/lusqsztk.js"></script>
    <lord-icon
