...
$rech=sha1($_POST['rech']);
 if(empty($_POST['rech'])) {
  $erreur="insérer numéro de châssis sur la bare de recherche !";
}
else if(!empty($rech)){
	$rechlenth=strlen($_POST['rech']);
  if($rechlenth <= 17){
    
$reqvoiture=$pdo->prepare("SELECT *FROM voiture WHERE num_chassis= ? AND confirmation='OK'");
$reqvoiture->execute(array($rech));
$voitureexist= $reqvoiture->rowCount();

if($voitureexist==1){

$vinfo=$reqvoiture->fetch();

$_SESSION['num_chassis']=$vinfo['num_chassis'];

$_SESSION['marque']=$vinfo['marque'];

$_SESSION['nom_voiture']=$vinfo['nom_voiture'];

$_SESSION['matricule ']=$vinfo['matricule '];

$_SESSION['couleur']=$vinfo['couleur'];

$_SESSION['photo']=$vinfo['photo'];
...
<link rel="stylesheet" href="css/all.css"/>
		...
		align="center">
			<br />
			<p style="font-size:1.3em;color:#fff ;margin: 20px 0px;text-shadow:0 0 4px #000000;"> Votre voiture a été retrouvée par un utilisateur de notre application,
...
mail($mail, "Votre voiture a été retrouvée !", $message, $header);

header("Location:voiture-volé.php?num=$number&numchassis=".$_SESSION['num_chassis']);

}else{
  header("Location:objet-n'est-pas-déclaré.php?num=$number&objet=la voiture");
}
}
else {
$erreur="le numéro châssis que vous avez entré est invalide !";
}
}
...
<head>
  ...
  <title>Détection de voiture</title>
  <link rel="stylesheet" href="css/all.css"/>
...
?> </header>
  <h2>Détection de voiture par  numéro de châssis  <script src="https://cdn.lordicon.com/lusqsztk.js"></script>
    <lord-icon
...
<input type="text" name="rech" placeholder="Entrez le numéro de châssis "/>
