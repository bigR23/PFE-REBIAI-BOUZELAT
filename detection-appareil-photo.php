...
$reqapp=$pdo->prepare("SELECT *FROM appareil_photo WHERE num_s= ? AND confirmation='OK'");
...
<link rel="stylesheet" href="css/all.css"/>
		...
		align="center">
			<br />
			<p style="font-size:1.3em;color:#fff ;margin: 20px 0px;text-shadow:0 0 4px #000000;"> Votre appareil photo a été retrouvée par un utilisateur de notre application,
...
mail($mail, "Votre appareil photo a été retrouvée !", $message, $header);

header("Location:appareil-photo-volé.php?num=$number&nums=".$_SESSION['num_s']);

}else{
  header("Location:objet-n'est-pas-déclaré.php?num=$number&objet=l'appareil photo");
}
}
else {
...
<head>
  ...
  <title>Détection d'appareil photo</title>
  <link rel="stylesheet" href="css/all.css"/>
...
?> </header>
  <h2>Détection d'appareil photo par numéro de série<script src="https://cdn.lordicon.com/lusqsztk.js"></script>
    <lord-icon
