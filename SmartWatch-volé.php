...
if(isset($_GET['imei'])){
	$getimei=$_GET['imei'];
	$req=$pdo->prepare("SELECT * FROM smart_watch WHERE imei= ?");
	$req->execute(array($getimei));
	$info=$req->fetch();
 if(isset($_SESSION['imei'])AND $info['imei']==$_SESSION['imei']){
  $ID=$info['id'];
...
<body>
  ...
  <section class="banner">
    <header>
      <a href="detection-SmartWatch.php?num=<?=$number?>" class="re" ><i id ="icon"class="fas fa-arrow-alt-circle-left"></i> Retour</a>
   
    </header>  
     <h2>attention la Smart Watch qui vous interesse a était déja volé  <script src="https://cdn.lordicon.com/lusqsztk.js"></script>
<lord-icon
...
<div class="container">
  ...
  <img  src="SmartWatch-déclarés/<?php echo $info['photo'];?>" height="280px" width="330px">
    <ul>
