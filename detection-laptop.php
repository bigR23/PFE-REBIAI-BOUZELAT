...
$rech=sha1($_POST['rech']);
 if(empty($_POST['rech'])) {
  $erreur="insérer un numéro de série sur la bare de recherche !";
}
else if(!empty($rech)){
	$rechlenth=strlen($_POST['rech']);
  if($rechlenth <=25 ){
    
$reqlap=$pdo->prepare("SELECT *FROM Laptop WHERE num_s= ? AND confirmation='OK' ");
$reqlap->execute(array($rech));
$lapexist= $reqlap->rowCount();

if($lapexist==1){

$lapinfo=$reqlap->fetch();

$_SESSION['id_modele']=$lapinfo['id_modele'];

$_SESSION['marque']=$lapinfo['marque'];

$_SESSION['nom_appareil']=$lapinfo['nom_appareil'];

$_SESSION['num_s']=$lapinfo['num_s'];

$_SESSION['couleur']=$lapinfo['couleur'];

$_SESSION['photo']=$lapinfo['photo'];
...
<link rel="stylesheet" href="css/all.css"/>
		...
		align="center">
			<br />
			<p style="font-size:1.3em;color:#fff ;margin: 20px 0px;text-shadow:0 0 4px #000000;"> Votre Laptop a été retrouvé par un utilisateur de notre application,
...
mail($mail, "Votre laptop a été retrouvé!", $message, $header);

header("Location:laptop-volé.php?num=$number&nums=".$_SESSION['num_s']);

}else{
  header("Location:objet-n'est-pas-déclaré.php?num=$number&objet=le Laptop");
}
}
else {
...
<head>
  ...
  <title>Détection de Laptop</title>
  <link rel="stylesheet" href="css/all.css"/>
...
?> </header>
  <h2>Détection du Laptop par numéro de série<script src="https://cdn.lordicon.com/lusqsztk.js"></script>
    <lord-icon
