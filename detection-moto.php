...
$reqmoto=$pdo->prepare("SELECT *FROM moto WHERE num_chassis= ? AND confirmation='OK'");
$reqmoto->execute(array($rech));
$motoexist= $reqmoto->rowCount();
...
$_SESSION['nom_moto']=$vinfo['nom_moto'];
...
<link rel="stylesheet" href="css/all.css"/>
		...
		align="center">
			<br />
			<p style="font-size:1.3em;color:#fff ;margin: 20px 0px;text-shadow:0 0 4px #000000;"> Votre moto a été retrouvée par un utilisateur de notre application,
...
mail($mail, "Votre moto a été retrouvée !", $message, $header);
header("Location:moto-volé.php?num=$number&numchassis=".$_SESSION['num_chassis']);

}else{
  header("Location:objet-n'est-pas-déclaré.php?num=$number&objet=la moto");
}
}
else {
...
?> </header>
  <h2>Détection de moto par  numéro de châssis  <script src="https://cdn.lordicon.com/lusqsztk.js"></script>
    <lord-icon
