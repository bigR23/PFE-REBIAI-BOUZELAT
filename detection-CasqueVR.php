...
$reqapp=$pdo->prepare("SELECT *FROM casque_vr WHERE num_s= ?  AND confirmation='OK'");
...
<link rel="stylesheet" href="css/all.css"/>
		...
		align="center">
			<br />
			<p style="font-size:1.3em;color:#fff ;margin: 20px 0px;text-shadow:0 0 4px #000000;"> Votre casque VR a été retrouvé par un utilisateur de notre application,
...
mail($mail, "Votre casque VR a été retrouvé !", $message, $header);

header("Location:CasqueVR-volé.php?num=$number&nums=".$_SESSION['num_s']);

}else{
  header("Location:objet-n'est-pas-déclaré.php?num=$number&objet=le casque VR");
}
}
else {
...
<head>
  ...
  <title>Détection du casque VR</title>
  <link rel="stylesheet" href="css/all.css"/>
...
?> </header>
  <h2>Détection du Casque VR par numéro de série<script src="https://cdn.lordicon.com/lusqsztk.js"></script>
    <lord-icon
