...
<html>
 ...
 <style type="text/css">
   ...
   height:35px;width: 150px;text-shadow: 0 0 7px rgb(0, 0, 0);
   margin-top: 20px;
   border:none;
  ...
  button::before {
   ...
   background:linear-gradient(to bottom,red,#FA7A7A);
...
<body>
  ...
  <section class="banner">
    ...
    <div class="alerte">
          ...
          <?php 
          if(isset($_GET['msg'])){
  echo '<p><i class="fa-solid fa-list-check" style="color:rgb(0,236,173);"></i> '."Le membre a était supprimée !"."</p>";
          } ?>
          </div> </header>
   
     <h2> la liste de tout les membres inscrit &emsp;<i id ="icon"class="fa-solid fa-clipboard-list"></i></h2>
    <div class="content">
  ...
  <?php 
     foreach(   $userinfo as   $user):
        ?>
        <li>
        <img src="img/<?php if ($user['genre']=='Homme') {echo ('boy.png');} else {echo ('girl.png');}?>" height="65px" style="position:relative;top:20px;"><a>ID=<?= $user['id'];?></a>:<a> <?=$user['pseudo'];?></a>
        &emsp;<a href="supprimer-membre.php?id=<?= $user['id'];?>&pseudo=<?= $user['pseudo'];?>&genre=<?= $user['genre'];?>"><button>supprimer</button><a>
      </li><br>   <hr>
   <?php endforeach  ?>
