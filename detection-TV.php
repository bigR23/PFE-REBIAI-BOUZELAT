...
$reqtv=$pdo->prepare("SELECT *FROM tv WHERE num_s= ? AND confirmation='OK'");
$reqtv->execute(array($rech));
$tvexist= $reqtv->rowCount();

if($tvexist==1){

$tvinfo=$reqtv->fetch();

$_SESSION['taille']=$tvinfo['taille'];

$_SESSION['marque']=$tvinfo['marque'];

$_SESSION['typeTV']=$tvinfo['typeTV'];

$_SESSION['num_s']=$tvinfo['num_s'];

$_SESSION['couleur']=$tvinfo['couleur'];

$_SESSION['photo']=$tvinfo['photo'];
...
<link rel="stylesheet" href="css/all.css"/>
		...
		align="center">
			<br />
			<p style="font-size:1.3em;color:#fff ;margin: 20px 0px;text-shadow:0 0 4px #000000;"> Votre TV a été retrouvé par un utilisateur de notre application,
...
mail($mail, "Votre TV a été retrouvé !", $message, $header);


header("Location:TVvolé.php?num=$number&nums=".$_SESSION['num_s']);

}else{
  header("Location:objet-n'est-pas-déclaré.php?num=$number&objet=la TV");
}
}
else {
...
?> </header>
  <h2>Détection de TV par numéro de série<script src="https://cdn.lordicon.com/lusqsztk.js"></script>
    <lord-icon
