...
if(isset($_POST['connexion']))
{
	...
	$motdepasseadmin=sha1($_POST['motdepasseadmin']);
		$reqadmin=$pdo->prepare("SELECT *FROM administration WHERE nomadmin= ? AND motdepasseadmin= ?");
		...
		if($adminexist==1){
			...
			header("Location:administration.php?nomadmin=".$_SESSION['nomadmin']);
			
		}
		else{
			$erreur="le nom admin ou le mot de passe est incorrect !";
		}
		
		
	}else{$erreur="complétez tous les champs s'il vous plait!";}
...
<div class="inputbx">
      <input type="password" name="motdepasseadmin"  placeholder="mot de passe">
    </div>
